...
fn test_create_image_file_info() {
    ...
    for (filename, hash, size, expected_type, expected_state) in test_cases {
        let path = PathBuf::from(filename);
        let info = ImageFileInfo::new(path.clone(), hash.to_string(), size);
...
fn test_equality_and_cloning() {
    let original = ImageFileInfo::new(PathBuf::from("test.jpg"), "testhash".to_string(), 100);
    ...
    // Test inequality
    let different = ImageFileInfo::new(
...
fn test_image_file_info_debug() {
    let info = ImageFileInfo::new(PathBuf::from("test.jpg"), "testhash".to_string(), 100);
...
fn test_image_state_transitions() {
    // Test initial states
    let tiff_image = ImageFileInfo::new(PathBuf::from("test.tiff"), "hash1".to_string(), 100);
    assert_eq!(tiff_image.image_state, ImageState::Tiff);

    let zero_byte_image = ImageFileInfo::new(PathBuf::from("test.jpg"), "hash2".to_string(), 0);
    assert_eq!(zero_byte_image.image_state, ImageState::ZeroByte);

    let normal_image = ImageFileInfo::new(PathBuf::from("test.png"), "hash3".to_string(), 100);
    assert_eq!(normal_image.image_state, ImageState::DuplicateCandidate);

    // Test transition to Unreferenced
    let mut info = ImageFileInfo::new(PathBuf::from("test.png"), "hash".to_string(), 100);
