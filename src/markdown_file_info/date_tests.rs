...
fn test_process_frontmatter_date_validation() {
    let test_cases = vec![
        DateValidationTestCase {
            ...
            file_system_mod_date: eastern_midnight(2024, 1, 15),
        ...
        DateValidationTestCase {
            ...
            date_modified: Some("[[2024-01-15]]".to_string()),
            date_created: Some("[[2024-01-15]]".to_string()),
            file_system_mod_date: eastern_midnight(2024, 1, 16),
            file_system_create_date: eastern_midnight(2024, 1, 16),
...
fn test_date_created_fix_integration() {
    let test_cases = vec![
        ...
        DateCreatedFixTestCase {
            ...
            expect_persist: true,
            expected_parsed_date: Some(eastern_midnight(2024, 1, 15)),
        },
        DateCreatedFixTestCase {
...
fn test_filesystem_timestamp_debug() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let file_path = TestFileBuilder::new()
        .with_fs_dates(test_date, test_date)
        .create(&temp_dir, "timestamp_test.md");

    // Get metadata immediately after creation
    let metadata = fs::metadata(&file_path)?;

    // Get different representations of the timestamps
    let created_time = FileTime::from_creation_time(&metadata)
        .map(|ft| {
            let system_time = SystemTime::UNIX_EPOCH + std::time::Duration::from_secs(ft.unix_seconds() as u64);
