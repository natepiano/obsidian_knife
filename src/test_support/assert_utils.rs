...
/// - `test_name`: A name or description of the test case, used for more informative error messages
///   on failure.
/// - `ok_compare`: A function or closure that takes references to the `Ok` values of `result` and
///   `expected`. It will be called to assert the equality of `Ok` values, and should panic if they
///   do not match.
///
/// # Panics
/// - If `result` and `expected` have different `Ok` or `Err` values, a detailed assertion message
///   will be printed, showing the test name, the expected value, and the actual value to facilitate
///   debugging.
/// - If `result` is `Ok` and `expected` is `Err` (or vice versa), it panics with a mismatch
///   message.
///
/// # Example
/// ```ignore
