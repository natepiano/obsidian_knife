...
use crate::constants::PHANTOM_LINK;
use crate::constants::PHANTOM_LINKS;
use crate::constants::PHANTOM_LINKS_DESCRIPTION;
use crate::constants::SOURCE_TEXT;
use crate::constants::WILL_REPLACE_WITH;
use crate::description_builder::DescriptionBuilder;
use crate::markdown_file::PhantomLinkMatch;
...
struct PhantomLinksTable;

impl ReportDefinition for PhantomLinksTable {
    type Item = PhantomLinkMatch;
    ...
    fn build_rows(
        ...
        let mut rows: Vec<Vec<String>> = items
            .iter()
            .map(|phantom_link_match| {
                let file_stem = Path::new(&phantom_link_match.relative_path)
                    .file_stem()
                    .and_then(OsStr::to_str)
                    .unwrap_or_default();

                // Table-sourced replacements already carry `ESCAPED_PIPE`.
                let replacement = if phantom_link_match.replacement.contains(ESCAPED_PIPE) {
                    phantom_link_match.replacement.clone()
                } else {
                    support::escape_pipe(&phantom_link_match.replacement)
                };

                vec![
                    file_stem.to_wikilink(),
                    phantom_link_match.line_number.to_string(),
                    support::escape_pipe(&support::escape_brackets(&phantom_link_match.found_text)),
                    replacement.clone(),
    ...
    fn title(&self) -> Option<String> { Some(PHANTOM_LINKS.to_string()) }
    ...
    fn description(&self, items: &[Self::Item]) -> String {
        ...
        DescriptionBuilder::new()
            ...
            .text_with_newline("")
            .no_space(PHANTOM_LINKS_DESCRIPTION)
            .build()
    }
...
impl ObsidianRepository {
    pub(super) fn write_phantom_links_report(
        &self,
        output_file_writer: &OutputFileWriter,
    ) -> Result<(), Box<dyn Error + Send + Sync>> {
        let matches: Vec<PhantomLinkMatch> = self
            .markdown_files
            .files_to_persist()
            .iter()
            .flat_map(|file| file.phantom_link_matches.clone())
            .collect();

        let report_writer = ReportWriter::new(matches);
        report_writer.write(&PhantomLinksTable, output_file_writer)
    }
}
