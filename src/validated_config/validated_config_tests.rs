...
fn test_invalid_back_populate_count() {
    ...
    let result = get_test_validated_config_result(&temp_dir, |builder| {
        builder.back_populate_file_count(Some(0));
    });

    assert!(matches!(
        result.unwrap_err(),
        ValidationError::InvalidBackPopulateCount
    ));

    // Test that valid counts work
    let result = get_test_validated_config_result(&temp_dir, |builder| {
        builder.back_populate_file_count(Some(1));
    });
    assert!(result.is_ok());
}
...
fn test_multiple_validation_errors() {
    ...
    let result = get_test_validated_config_result(&temp_dir, |builder| {
        builder
            .back_populate_file_count(Some(0))
...
fn test_all_validation_passes() {
    ...
    let result = get_test_validated_config_result(&temp_dir, |builder| {
        builder
            .back_populate_file_count(Some(1))
